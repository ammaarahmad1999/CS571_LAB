...
def print_sol(src, dest, hash_map):
    ...
    if src == reverse_hash(dest): # destination reached
        print_aux(src)
        return
    #value = hash_value(dest)
    temp = hash_map[dest][0]
    print_sol(src, temp, hash_map) # do a recursive call
    print_aux(reverse_hash(dest))
...
def print_results(src, dest, hash_map, cnt, start, end, last_cur):
    ...
    if tmp in hash_map:     # if solution is found
        ...
        print('Failure: Found a local maxima')
        print_aux(last_cur)
...
last_cur, hash_map, cnt = hill_climbing(src, dest, h1)
